...
- **Brand variants** — `pui-acorn:`, `pui-sprig:`, `pui-stelpro:`, `pui-cima:` via `@custom-variant`
...
Brand-specific design tokens (CSS custom properties) for the Sprig brand, layered on top of the Acorn base. Each brand file imports `acorn.css` + its own overrides.
...
Each brand only defines values that DIFFER from the Acorn base. Sprig overrides ~30 tokens; everything else is inherited. Brand tokens are defined as JSON in `@perimetre/tokens` and generated into CSS:

```css
/* Generated from packages/tokens/src/sets/brands/sprig.json */
@layer pui.semantic {
...
To add a new brand, start in `packages/tokens` (create JSON + generate CSS), then wire it up here. See [CONTRIBUTING.md](./CONTRIBUTING.md) for the full guide.
...
- `@perimetre/tokens` — Design tokens (JSON source of truth + generated CSS)
